...
bool Response::validateMethod()
{
	...
	return valid_methods.find(getMethod()) != valid_methods.end();
}

void Response::handlerValidRequest(Logger &logger)
{
    if (!validateMethod() && !getMethod().empty())
    {
...
void Response::processRequest(const std::string &raw_request, const ServerConfigs* respconfig, Logger &logger)
{
    (void)respconfig;
    std::string path;

    if (!parseRequest(raw_request))
    {
        std::cout << "debbug response 1" << std::endl;
        handlerValidRequest(logger);
        return;
    }

    if (getUri().find(_root) != std::string::npos)
    {
